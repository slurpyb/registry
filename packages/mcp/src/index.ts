#!/usr/bin/env bun
/**
 * MCP server for OCX skill registry discovery.
 * Fetches data from the deployed Cloudflare Workers registry.
 * Transport: stdio
 */
...
const BASE_URL = "https://registry.slurpyb.workers.dev/_data"
...
try {
  ;[registry, domains] = await Promise.all([
    fetchJSON<EnrichedEntry[]>(`${BASE_URL}/enriched-registry.json`),
    fetchJSON<DomainSummary>(`${BASE_URL}/domains.json`),
  ])
} catch (err) {
